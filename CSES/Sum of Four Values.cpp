...
gp_hash_table<ll,int, custom_hash> ump;
...
//	ofstream cout("/Users/konwoo/output.txt");
    ...
    F0R(i,n){
        FOR(j,i+1,n){
            vec.pb(mp(arr[i]+arr[j],mp(i,j)));
        }
    }
    bool found = 0;
    F0R(i, sz(vec)){
        if (found) break;
        if (ump.find(x - vec[i].f) != ump.end()){
            set<int> chck;
            chck.insert(vec[i].s.f);
            chck.insert(vec[i].s.s);
            chck.insert(vec[ump[x-vec[i].f]].s.f);
            chck.insert(vec[ump[x-vec[i].f]].s.s);
            if (sz(chck) == 4){
                found = 1;
                for (auto &b : chck) cout << b + 1<< " ";
                break;
            }
        }
        ump.insert(mp(vec[i].f, i));
    }
